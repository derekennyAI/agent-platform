...
if [ -n "$PRO_TOKEN" ]; then
    ...
    $TMUX new-session -d -s "$SESSION" -c "/Users/YOUR_MAC_USERNAME/{name}" \\
        "env -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN AGENT_NAME=$AGENT_NAME SUPABASE_URL=$SUPABASE_URL SUPABASE_SERVICE_KEY=$SUPABASE_SERVICE_KEY CLAUDE_CODE_OAUTH_TOKEN=$PRO_TOKEN TELEGRAM_STATE_DIR={channel_dir} $CLAUDE --dangerously-skip-permissions --mcp-config /Users/YOUR_MAC_USERNAME/{name}/.mcp.json --settings /Users/YOUR_MAC_USERNAME/{name}/settings.json --channels plugin:telegram@claude-plugins-official --append-system-prompt 'CRITICAL STARTUP GATE: Before responding to ANY channel messages, you MUST first read and execute all instructions in ~/{name}/startup-instructions.md. Complete every step before handling user requests.'"
else
    infra_warn "$COMP" "Pro token expired or missing — falling back to API key (Sonnet)"
    $TMUX new-session -d -s "$SESSION" -c "/Users/YOUR_MAC_USERNAME/{name}" \\
        "env -u ANTHROPIC_AUTH_TOKEN AGENT_NAME=$AGENT_NAME SUPABASE_URL=$SUPABASE_URL SUPABASE_SERVICE_KEY=$SUPABASE_SERVICE_KEY TELEGRAM_STATE_DIR={channel_dir} $CLAUDE --model claude-sonnet-4-6 --dangerously-skip-permissions --mcp-config /Users/YOUR_MAC_USERNAME/{name}/.mcp.json --settings /Users/YOUR_MAC_USERNAME/{name}/settings.json --channels plugin:telegram@claude-plugins-official --append-system-prompt 'CRITICAL STARTUP GATE: Before responding to ANY channel messages, you MUST first read and execute all instructions in ~/{name}/startup-instructions.md. Complete every step before handling user requests.'"
fi
